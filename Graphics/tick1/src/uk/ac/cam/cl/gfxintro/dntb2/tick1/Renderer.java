...
public class Renderer {
	...
	protected ColorRGB trace(Scene scene, Ray ray, int bouncesLeft) {
		...
		if (bouncesLeft == 0 || reflectivity == 0) {
			...
			Vector3 L = ray.getDirection().scale(-1).normalised();
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		ColorRGB ambientColour = C_diff.scale(I_a);
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			Vector3 R = N.scale(2 * L.dot(N)).subtract(L);

			ColorRGB diffuseColour = C_diff.scale(I).scale(k_d * Math.max(0, N.dot(L)) );
			ColorRGB specularColour = C_spec.scale(I).scale(k_s * Math.pow( Math.max(0, R.dot(V)) , alpha) );

			// TODO: Calculate L, V, R
			// TODO: Calculate ColorRGB diffuse and ColorRGB specular terms
			// TODO: Add these terms to colourToReturn

			RaycastHit intersect = scene.findClosestIntersection(new Ray(P.add(N.scale(EPSILON)), L));

			if (intersect.getObjectHit() == null || intersect.getDistance() > distanceToLight) {
				colourToReturn = colourToReturn.add(diffuseColour).add(specularColour);
			}
			//colourToReturn = colourToReturn.add(ambientColour).add(diffuseColour).add(specularColour);

		}
		return colourToReturn.add(ambientColour);
	}
