...
from tick2 import calculate_class_log_probabilities, calculate_smoothed_log_probabilities, predict_sentiment_nbc
...
def read_lexicon_magnitude(filename: str) -> Dict[str, Tuple[int, str]]:
    ...
    with open("data/sentiment_detection/sentiment_lexicon", "r") as f:
        lexicon = {}
        for line in f:
            ...
            intensity = line[1][10:]
            lexicon[word] = (polarity, intensity)
    return lexicon
...
def sign_test(actual_sentiments: List[int], classification_a: List[int], classification_b: List[int]) -> float:
    ...
    for i in range(len(actual_sentiments)):
        if actual_sentiments[i] == classification_a[i] and actual_sentiments[i] == classification_b[i]:
            ties += 1
        elif actual_sentiments[i] == classification_a[i]:
            plus += 1
        elif actual_sentiments[i] == classification_b[i]:
            minus += 1
        else:
            ties += 1
            
            
    n = 2*(math.ceil(ties/2)) + minus + plus
    
    k = math.ceil(ties/2) + min(minus, plus)
